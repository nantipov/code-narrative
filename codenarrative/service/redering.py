...
def render(scene: domain.scene.Scene, profile_name: str):
    ...
    # service.html.write_styles(location)
    ...
    for keyframe in scene.timeline:
        ...
        while state.frame < keyframe.frame - 1:
            #service.html.render_html(location, state)
        ...
        if not keyframe.code is None:
            ...
            keyboard_actions = service.keypress.compute_keypresses(state.cursor, text1, text2)
        ...
        if len(keyframe.screen_objects) > 0:
            for obj in keyframe.screen_objects:
                if obj.action == "add" and not obj.id in state.screen_objects: #todo: enum
                    state.screen_objects[obj.id] = domain.rendering.ObjectAnimationState(obj)
            ...
            for object_frame in range(animation_frames + 1):
                for obj in keyframe.screen_objects:
                    ...
                    match obj.action:
                        case "add":
                            animation_state.animation_progress = round(object_frame * 100 / animation_frames)
                        case "remove":
                            animation_state.animation_progress = round((animation_frames - object_frame) * 100 / animation_frames)
                service.png.render_png(location, state)
                state.frame = state.frame + 1
            for obj in keyframe.screen_objects:
                match obj.action:
...
def animate_keypresses(state: domain.rendering.SceneState, location: domain.storage.Location, keypresses: list[domain.keypress.Keypress]):
        ...
        match keypress.key:
            case domain.keypress.Key.OTHER:
                if state.cursor.index >= len(state.code.text):
                    ...
                    if state.cursor.is_insert:
                        state.code.text = state.code.text[:state.cursor.index] + keypress.char + state.code.text[state.cursor.index:]
                    else:
                        state.code.text = state.code.text[:state.cursor.index] + keypress.char + state.code.text[state.cursor.index+1:]
            ...
            case domain.keypress.Key.ARROW_LEFT | domain.keypress.Key.ARROW_RIGHT | domain.keypress.Key.ARROW_UP | domain.keypress.Key.ARROW_DOWN:
                if is_max_cols_changed:
                    max_cols = service.cursor.get_max_cols(state.code.text)
                    is_max_cols_changed = False
                service.cursor.move_cursor(max_cols, state.cursor, keypress.key, keypress.aligned_current_column)
            ...
            case domain.keypress.Key.BACKSPACE:
                ...
                if state.cursor.index >= len(state.code.text):
                    ...
                    state.code.text = state.code.text[:state.cursor.index-1] + state.code.text[state.cursor.index:]
...
def keypress_jitter_frames(state: domain.rendering.SceneState) -> int:
    ...
    if state.cursor.pos.col == 1:
        return jitter_f
    if not state.code is None and len(state.code.text) > state.cursor.index and state.code.text[state.cursor.index] in jitter_chars:
        return jitter_f
    return 0

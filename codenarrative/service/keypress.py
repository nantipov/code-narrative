    ...
    for code in codes:
        match code.tag:
            case "replace":
                ...
                keypresses.append(
                    domain.keypress.Keypress(domain.keypress.Key.TOGGLE_CURSOR_INSERT)
                )
...
def bring_cursor(
    current_cursor: domain.rendering.Cursor, max_cols: list[int], target_index: int
) -> list[domain.keypress.Keypress]:
    ...
    while target_index != current_cursor.index:
        ...
        line_min_index = (
            current_cursor.index - current_cursor.pos.col + 1

...
def profile_by_name(scene: domain.scene.Scene, profile_name: str) -> domain.scene.Profile:
    default_profile = builtin_default_profile
    if len(scene.profiles) == 1:
        default_profile = scene.profiles[0]
    elif len(scene.profiles) > 1:
        default_profile = next((p for p in scene.profiles if p.is_default), scene.profiles[0])

...
from pygments.token import Token, Name, Operator, Keyword, Generic, Comment, \
    Number, String, Literal, Punctuation, Error, _TokenType
...
def render_all_pngs(location: domain.storage.Location, state: domain.rendering.SceneState):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers = workers_quantity) as pool:
        for w in workers:
            ...
            w.future(f)
    concurrent.futures.wait(list(map(lambda w: w.fut, workers)), timeout=None, return_when=concurrent.futures.ALL_COMPLETED)
...
def render_png(location: domain.storage.Location, state: domain.rendering.SceneState):
    ...
    im = Image.new(mode="RGBA", size=(640, 480), color="#3f3f3f") #todo: add styles, background to the scene file
    ...
    font = ImageFont.truetype("fonts/AzeretMono-Medium.ttf", size=text_size) #todo: assets directory fonts, sounds?
    ...
    if not state.code is None:
        ...
        for token_type, token_value in lexer.get_tokens(text=state.code.text):
            ...
            if token_value != "\n":
                draw.text(xy=(current_char_x, current_char_y), text=token_value, font=font, fill=token_color(token_type))

            if token_value == "\n":
                # new line symbol debug
                if state.profile.is_debug:
                    draw.text(xy=(current_char_x, current_char_y), text="n", font=font, fill="#bf00ff")
                current_char_x = 0
    ...
    for _, obj_state in state.screen_objects.items():
        obj = obj_state.obj
        alpha_value = round(obj_state.animation_progress * 128 / 100) #todo: declare as constant 128-max opactity?
        ...
        # object debug
        if state.profile.is_debug:
            draw.text(xy=(100, 400), text=f"progress: {obj_state.animation_progress}; alpha: {alpha_value} : {obj.background_color + alpha_color}", fill="#ffffff")


    # cursor position debug
    if state.profile.is_debug:
        draw.text(xy=(0, 400), text=f"{state.cursor.pos.col} x {state.cursor.pos.row}: {state.cursor.index}", fill="#ffffff")
